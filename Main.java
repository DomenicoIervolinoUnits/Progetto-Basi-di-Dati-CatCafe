...
public class Main {
    ...
    private static void visualizzaTavoli() {
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            System.out.println("Situazione dei tavoli");
            while (rs.next()) {
    ...
    private static void cambiaStatoTavolo(Scanner scanner) {
        ...
        System.out.print("Imposta il nuovo stato (0 = Occupato, 1 = Libero): ");
        int nuovoStatoInt = scanner.nextInt();

        if (nuovoStatoInt != 0 && nuovoStatoInt != 1) {
            System.out.println("Lo stato inserito non è valido.");
            return;
        }

        boolean nuovoStato = (nuovoStatoInt == 1);
        String query = "UPDATE Tavolo SET Stato = ? WHERE NumeroTavolo = ?";

        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
             PreparedStatement pstmt = conn.prepareStatement(query)) {

            pstmt.setBoolean(1, nuovoStato);
